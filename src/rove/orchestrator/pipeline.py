...
class EvaluationPipeline:
    ...
    def _validate_evidence_consistency(
        ...
        """
        verification = verification.model_copy(update={"dynamics_analysis": dynamics})
        ...
        if dynamics is None:
            ...
            has_violations = (
                _get_evidence_value(dynamics, "joint_limits_ok") is False
                or _get_evidence_value(dynamics, "self_collision") is True
                or _get_evidence_value(dynamics, "torque_feasible") is False
            )
            ...
            tools.append(
                {
                    ...
                    "description": (
                        ...
                        "Returns structured evidence categorized as 'hard' (computed for the supplied model/state) "
    ...
    def _compile_physics_evidence(self, dynamics_analysis: dict | None) -> str:
        ...
        for item in evidence:
            ...
            if field in (
                ...
                if value is None:
                    status = "UNKNOWN / NOT COMPUTED"
                elif field in ("self_collision", "near_singularity"):
                    status = "DETECTED" if value else "NOT DETECTED"
                else:
                    status = "OK" if value else "VIOLATED"
                status += f" [confidence: {item.get('confidence', 'unknown')}]"
        ...
        if summary:
            ...
            if (summary.get("peak_torque_nm") or 0) > 0:
