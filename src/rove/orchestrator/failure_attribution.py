...
def _get_evidence_value(dyn: dict, field: str) -> bool | None:
    ...
    """
    if any(item.get("field") == field for item in dyn.get("not_computed", [])):
        return None
    # Structured evidence is authoritative; never bypass uncertainty via summary.
    evidence = dyn.get("evidence", [])
    for item in evidence:
        if item.get("field") == field:
            value = item.get("value")
            return value if item.get("confidence") == "hard" and isinstance(value, bool) else None
    if "evidence" in dyn:
        return None
...
def attribute_failure(
    ...
    if dyn is None:
        verify_output = (stage_map.get("verify") or {}).get("output") or {}
        dyn = verify_output.get("dynamics_analysis")
        if dyn:
            dyn_stage_name = "dynamics"
    if dyn:
