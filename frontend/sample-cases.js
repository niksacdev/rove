...
function sampleCaseHref(record) {
  ...
  return `/static/datasets.html?case=${encodeURIComponent(revision)}`;
}

function createSampleCaseCard(record, onQuickRun, apiBase = "", doc = document) {
  ...
  const quick = node("button", null, "flex items-center gap-3 w-full p-2 rounded-lg hover:bg-f-surface/80 transition-colors text-left group");
  quick.type = "button";
  quick.setAttribute("aria-label", `Quick run: ${record.task}`);
  quick.addEventListener("click", () => onQuickRun(record));
  ...
  info.appendChild(node("span", "Quick run →", "text-[10px] text-f-purple block mt-1"));
  quick.appendChild(image); quick.appendChild(info); card.appendChild(quick);
  const footer = node("div", null, "px-3 py-2 border-t border-f-border text-[10px]");
  const href = sampleCaseHref(record);
  if (href) {
    const link = node("a", "Open case →", "text-f-purple hover:text-white");
    link.href = href;
    link.setAttribute("aria-label", `Open versioned case: ${record.task}`);
    footer.appendChild(link);
  } else {
    footer.appendChild(node("span", "Versioned case unavailable", "text-gray-500"));
