...
from rove.config import find_model_config, get_all_models, get_model_config
...
def _build_adapter(
    ...
    model_cfg = get_model_config(model_type, model_id)
    adapter_name = model_cfg.get("adapter", "")

    if adapter_name not in adapter_map:
        raise ValueError(
            f"No {model_type} adapter implementation for '{adapter_name}'. "
...
class AdapterRegistry:
    ...
    def get_vlm(self, model_id: str) -> VLMAdapter:
        return _build_adapter("vlm", model_id, _VLM_ADAPTERS, self._vlm_cache)

    def get_policy(self, model_id: str) -> PolicyAdapter:
        return _build_adapter("vla", model_id, _POLICY_ADAPTERS, self._policy_cache)

    def get_agent(self, model_id: str) -> AgentAdapter:
        return _build_adapter("agent", model_id, _AGENT_ADAPTERS, self._agent_cache)
    ...
    def get_adapter_for_stage(
        ...
        model_type, model_cfg = find_model_config(model_id)
        ...
        if model_type == "agent":
            ...
            if required and required not in capabilities:
                raise ValueError(
                    ...
                    f"Its capabilities: {capabilities}"
                )
        ...
        if stage == PipelineStage.ACT:
            if model_type != "vla":
                raise ValueError(
                    f"Model '{model_id}' (type={model_type}) cannot serve 'act'. "
        ...
        if model_type not in ("vlm", "llm"):
            raise ValueError(
                f"Model '{model_id}' (type={model_type}) cannot serve '{stage.value}'. "
