...
| Import ABC observations | `rove data case import-abc DIRECTORY --episode-id ID --source-revision REV --preview`, then the same command with `--preview-hash HASH` | Upstream converted metadata, camera video and state/action files become an ordinary sourced case. Split, domain, frame and camera are explicit; [example](../../examples/abc/README.md) |
...
| Assistant provider setup | `rove assistant settings`, `configure --provider copilot`, `configure --endpoint ID`, `configure --provider foundry --foundry-endpoint URL --deployment NAME`, `test`, `disable` | Uses the owning local server; [setup and verification limits](assistant-setup.md). New default never overwrites saved local/disabled selection |
| Copilot sign-in and discovery | `rove assistant login`, `login-status ID`, `cancel-login ID`, `auth-status`, `models` | Explicit dedicated ROVE profile; no automatic reuse of GitHub CLI credentials; discovery is separate from inference |
